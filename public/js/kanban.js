...
function renderKanban() {
  ...
  statuses.forEach(st => {
    const cards = filtered.filter(t => t.status === st);
    $(`count-${st}`).textContent = cards.length;
    $(`cards-${st}`).innerHTML = cards.map(t => renderKanbanCard(t)).join('');
  });
...
function renderKanbanCard(t) {
  ...
  return `<div class="${classes.join(' ')}" data-id="${t.id}" draggable="true">
    ...
    <div class="card-meta">${tagsHtml}${dueHtml}<span style="font-size:10px;color:var(--text-tertiary)">${t.estimate || 30}мин</span></div>
